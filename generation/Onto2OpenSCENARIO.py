...
for i in range(1, size):
    # print(i)
    wholepath.append("file://./" + sys.argv[i])
    print(wholepath[i - 1])
    template_ontology.append(get_ontology(wholepath[i - 1]).load())
    print(template_ontology[i - 1])
...
class Scenario(ScenarioGenerator):
    ...
    def __init__(self):
        ...
        d1 = {}
        d1['Scenario'] = "_" + str(current_time.tm_hour) + "_" + str(current_time.tm_min) + "_" + str(current_time.tm_sec)
    ...
    def scenario(self, **kwargs):
        scenario_instances = template_ontology[0].Scenario.instances()
        print(len(scenario_instances))
        # 此部分功能为实现多个场景结合
        if len(template_ontology) > 1:
            main_scenario = template_ontology[0].Scenario("main_scenario")
            main_init = template_ontology[0].Init("main_init")
            main_storyboard = template_ontology[0].Storyboard("main_storyboard")
            for template_onto in template_ontology[1:]:
                buffer_scenario = template_onto.Scenario("buffer_scenario")
                for scenario_in_onto in scenario_instances:
                    # print(scenario_in_onto.has_entity)
                    for scenario_entities in scenario_in_onto.has_entity:
                        main_scenario.has_entity.append(scenario_entities)
                    # print(scenario_in_onto.has_storyboard)
        ...
        ego_vehicles = template_ontology[0].EgoVehicle.instances()
